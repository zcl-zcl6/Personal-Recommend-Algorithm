...
import numpy as np
import sys
sys.path.append("F:/Pycharm/util")
import read as read
def lfm_train(train_data,F,alpha,beta,step) :
    ...
    Args:
        ...
        step: iteration num
    Return:
        dict: key userid,value vector(a list)
        dict: key itemid,value vector(a list)
    """
    user_vec = {}
    item_vec = {}
...
def model_train_process():
    ...
    """
    train_data = read.get_train_data(r"F:\Pycharm\data\ratings.txt")
...
def ana_recom_result(train_data,userid,recom_list):
    ...
    """
    item_info = read.get_item_info(r"F:\Pycharm\data\movies.txt")
    for data_instance in train_data:
...
# def train_true_rate(user_vec,item_vec,train_data):
#     real = 0
#     predict = 0
#     for data in train_data:
#         userid,itemid,label = data
#         temp = model_predict(user_vec[userid],item_vecitemid)
